...
### 03/09 CHAMOUARD (interne)

Moins encombrée ce matin.

Bio : CRP 8.6 mg/L, baisse des PNN 7.27 G/L, DFG 49 G/L, iono N
TA 180/83 : ajout ESIDREX le matin

### 04/09 CHAMOUARD (interne)

Amélioration clinique. Pas de crachats.

Poursuite cortico pour 5 jours au total.

### 05/09 CHAMOUARD (interne)

Stabilité clinique.

Bio avec négativation totale de la CRP, le reste est stable.

ECBC stérile.

TA 197/82 le matin reconstrôlée à 120/56 après l'ESIDREX du matin. Je décale la dose du soir de 18 à 20h.

### 06/09 CHAMOUARD (interne)

Ajout IRBESARTAN pour avoir COAPROVEL

## 10/09 F. BEAUFILS

Stabilité clinique du 7 au 9/09.

Nous avons tenté un arrêt de la corticothérapie avec récidive immédiate du tableau spastique avec impossibilité d'expectoration

Reprise depuis le 09/09 de prednisolone 60 mg.

Ce jour, nette amélioration du frein expiratoire, et la patiente reprend des expectorations correctes sur son alpha 300.

Bio et gazométrie demain avec sérologie aspergillaire qui n'avait pas été réalisée.

On part sur un RAD jeudi 12/09 et sevrage corticothérapie.

Souhaite une vaccination anti VRS pour l'hiver que je lui prescris, la patiente sait qu'elle devra le payer de sa poche selon les recommandations en vigueur.

De même je remets une vaccination anti-pneumococcique qui date de plus de 5 ans.

L'ordonnance des traitements de sortie est déjà prête, à modifier si besoin.

**11/09 CHAMOUARD (interne)**

Va bien.

Gazométrie : pH 7.42, pCO2 49 mmHg, pO2 81 mmHg
Bio : DFG stable à 47 mL/min, CRP 9.9 mg/L, cholestase sans hyperbilirubinémie

Interne MATHILDE CHAMOUARD
"""
)
...
def make_nlp_for_text(raw_text: str, min_score: float = 0.1):
    """
...
def _is_finite(tok) -> bool:
    return tok is not None and tok.pos_ in {"VERB", "AUX"} and "VerbForm=Fin" in tok.morph

def _aux_chain_recursive(tok) -> list:
    """
...
def _get_governing_token(event_head):
    """
...
def _classify_tense(governor) -> TenseClass:
    """
    Tense logic:
