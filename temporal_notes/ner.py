...
# 2) NLP components
# ---------------------------
tokenizer = AutoTokenizer.from_pretrained("medkit/DrBERT-CASM2", use_fast=True)
hf_ner = pipeline(
    "token-classification",
    model="medkit/DrBERT-CASM2",
    tokenizer=tokenizer,
    aggregation_strategy="simple",
    # device=-1,  # uncomment to force CPU if MPS/CUDA causes issues
)
...
# ---------------------------
def sentences_spans(doc) -> List[Tuple[int,int]]:
...
def tlinks_timex_timex(timex: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for i in range(len(timex)):
        ...
        for j in range(i + 1, len(timex)):
            if parsed[j] is None:
                continue
            rel = compare_dates(parsed[i], parsed[j])
...
def process_markdown_file(md_path: Path, out_dir: Path, preview_chars: int = 10000) -> None:
    ...
    for s in doc.spans.get("dates", []):
        norm_obj = s._.get("date")  # AbsoluteDate / RelativeDate / Duration / str / None
        kind = "unknown"
        parts = {"y": None, "m": None, "d": None}
        rel_seconds = None

        if hasattr(norm_obj, "mode"):  # eds objects
            mode = getattr(norm_obj, "mode", None)
            if str(mode) == "absolute":
                kind = "absolute"
                parts["y"] = getattr(norm_obj, "year", None)
                parts["m"] = getattr(norm_obj, "month", None)
                parts["d"] = getattr(norm_obj, "day", None)
            elif str(mode) == "relative":
                kind = "relative"
                try:
                    td = norm_obj.to_duration()  # timedelta
                    rel_seconds = td.total_seconds()
                except Exception:
                    rel_seconds = None
        ...
        timex.append({
            "text": s.text,
            "norm": str(norm_obj) if norm_obj is not None else None,  # readable, JSON-safe
