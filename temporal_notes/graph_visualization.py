...
def build_graph(data: dict):
    ...
    events = [e for e in data.get("events", []) if not e.get("family")]
    timex_list = data.get("timex", [])
    timex_map = {t["id"]: t for t in timex_list}

    # Timex référencés
    referenced_timex_ids = set()
    ...
    anchor_map = {e["id"]: e.get("anchor_date") for e in events}
    keep_before = {
        (r["source"], r["target"])
        for r in data.get("relations", [])
